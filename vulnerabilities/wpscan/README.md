...
- `Dockerfile` — `golang:1.22-alpine` builder → `alpine:3.20` non-root; multi-stage copies `/usr/local/bin/wpscan` from `wpscanteam/wpscan:3.8.25`
- `testdata/fake-wpscan/` — test stub that outputs canned JSON; driven by `FAKE_MODE` env

## Testing

```bash
go test ./vulnerabilities/wpscan/... -v -count=1
go vet ./vulnerabilities/wpscan/...
```
